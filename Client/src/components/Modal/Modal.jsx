...
const Modal = ({ modalProps }) => {
  ...
  const buttonHandler = async (event) => {
    ...
    if (response && setTotalEdits) {
      ...
      setInventoryArray(inventoryArray.filter((object) => object.id !== id));
    }
  };
  ...
  return (
    <dialog ref={dialogRef} className="modal">
      <form method="dialog" className="modal__form">
        <h1>{question}</h1>
        <p className="modal__body">{disclaimer}</p>
        <div className="modal__buttons">
          <CancelButton className="button-cancel" dialogRef={dialogRef} />
          <Button
